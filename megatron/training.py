...
def train_step(forward_step_func, data_iterator,
    ...
    if args.deepspeed:
        print_rank_0("deepspeed model step")
        model.step()
    else:
        optimizer.step()
...
def flops_calculator(model, args, iteration_time):
    ...
    print_rank_0(f"Effective Tera Flops per GPU: giga_flops_per_model_per_train_step {giga_flops_per_model_per_train_step} {round(effective_tera_flops_per_gpu, 2)} and total parameters {round(approx_parameters_in_billions, 3)} B")
...
def train(forward_step_func, model, optimizer, lr_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        if args.save and args.save_interval and \
           iteration % args.save_interval == 0 and False:
            save_checkpoint(iteration, model, optimizer, lr_scheduler)
